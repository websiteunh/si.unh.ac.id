...
class Dokumen extends CI_Controller {
	...
	var $page		=		"front/dokumen";
	var $file		=		"dokumen";
	var $pk     =   "id_dokumen";
	var $title  =   "Dokumen";
	var $bread	=   "<ol class='breadcrumb'>
	<li class='breadcrumb-item'><a>Master</a></li>										
	<li class='breadcrumb-item active'><a href='front/dokumen'>Dokumen</a></li>										
	</ol>";				          
	...
	{
		$list = $this->m_dokumen->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $isi) {			
      ...
      if($isi->status=="draft"){      
      	...
      	$status = "<label class='badge badge-success'>Published</label>";
			...
			$row = array();
			$row[] = $no;
			$row[] = "<a href='front/dokumen/detail?id=$isi->id_dokumen'>$isi->judul</a> $status";			
			$row[] = "<img src='assets/uploads/artikel/$foto' class='mb-6 mw-100 rounded'>";
			$row[] = "
						<a href=\"front/dokumen/delete?id=$isi->id_dokumen\" onclick=\"return confirm('Anda yakin?')\" class=\"btn btn-danger btn-sm\" title=\"Hapus\"><i class=\"fa fa-trash\"></i></a>                          
            <a href=\"front/dokumen/edit?id=$isi->id_dokumen\" class=\"btn btn-primary btn-sm\" title=\"Edit\"><i class=\"fa fa-edit\"></i></a>";	
			$data[] = $row;
		}

		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->m_dokumen->count_all(),
						"recordsFiltered" => $this->m_dokumen->count_filtered(),
						"data" => $data,
				);
	...
	{		
		...
		echo "<meta http-equiv='refresh' content='0; url=".base_url()."front/dokumen'>";
	}		
    ...
    if(!empty($_FILES['gambar']['name'])){
			...
			if(!$this->upload->do_upload('gambar')){
				...
				$err = "";
				$row = $this->m_admin->getByID("md_dokumen","id_dokumen",$id)->row();
	    	if(isset($row->gambar)){
    ...
    if(!empty($_FILES['file']['name'])){
			...
			if(!$this->upload->do_upload('file')){
				...
				$row = $this->m_admin->getByID("md_dokumen","id_dokumen",$id)->row();
	    	if(isset($row->file)){
	    		unlink('assets/uploads/artikel/'.$row->file);         	    		
	    	}
				$data['file']	= $this->upload->file_name;
			}
		}
